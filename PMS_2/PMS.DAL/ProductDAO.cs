        ...
        public static int Save(ProductDTO dto)
        {
            try
            {
                using (DBHelper helper = new DBHelper())
                {
                    ...
                    if (dto.ProductID > 0)
                    {
                        sqlQuery = String.Format("Update Products Set Name='{0}',Price='{1}',PictureName='{2}',ModifiedOn='{3}',ModifiedBy='{4}' Where ProductID={5}",
                    ...
                    {
                        sqlQuery = String.Format("INSERT INTO Products(Name, Price, PictureName, CreatedOn, CreatedBy,IsActive) VALUES('{0}','{1}','{2}','{3}','{4}',{5})",
        ...
        public static String[] GetUserNames(List<ProductDTO> pro)
        {
            ...
            try
            {
                using (DBHelper helper = new DBHelper())
                {
                    ...
                    {
                        String query = String.Format(@"Select Name from users where userID = {0}", pro[i].CreatedBy);
